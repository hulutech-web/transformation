...
export default {
  ...
  methods: {
    async showCarInfo() {
      this.form = await this.axios.get(`admin/carinfo/${this.id}`)
    },
    onSubmit() {
      this.$refs.ruleForm.validate(async valid => {
        if (valid) {
          await this.axios.put(`admin/carinfo/${this.id}`, this.form)
          await this.showCarInfo()
        } else {
