  ...
  <div>
    <a-card title="合同管理">
      ...
      >
        ...
        <a-table :dataSource="dataSource.data" :pagination="false" :columns="columns" rowKey="id" size="small"
                 bordered>
          <template slot="operation" slot-scope="text,record">
            <a-button type="primary" @click="select(record)" size="small">选择</a-button>
          </template>
        </a-table>
        <a-pagination v-if="dataSource.meta && dataSource.meta.total>dataSource.meta.per_page"
                      :defaultCurrent='1'
...
export default {
  ...
  methods: {
    async getContract() {
      this.formState = await this.axios.get(`admin/contract/${this.id}`)
    },
    ...
    //生成编码
    async makeBarCode() {
      let res = await this.axios.post('admin/contract/test')
      this.$message.success('已生成')
      this.formState.uuid = res
      JsBarcode("#barcode", res, {
        format: "CODE128", //选择要使用的条形码类型
        width: 1, //设置条之间的宽度
        height: 20, //高度
        displayValue: false, //是否在条形码下方显示文字
        textAlign: "center", //文字所在位置,默认中间
        text: res, //条形码下面显示的文本内容
        lineColor: "#000", //设置条和文本的颜色。
      })
    },
    async onSubmit() {
      await this.axios.put(`admin/contract/${this.id}`, this.formState).then(_ => {
        //清空formState
        this.formState = {}
        this.$router.push({name: 'admin.contract.index'})
      })
    },
