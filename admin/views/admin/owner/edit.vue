...
export default {
    ...
    methods: {
        ...
        async getOwner() {
            this.form = await this.axios.get(`admin/owner/${this.id}`)
        },
        async getBuilds() {
        ...
        onSubmit() {
            this.$refs.ruleForm.validate(async valid => {
                if (valid) {
                    ...
                    await this.axios.put(`admin/build/${this.form.build_id}/unit/${this.form.unit_id}/owner/${this.id}`, this.form)
                    //清空formState
                    this.form = {}
                    //跳转回列表页
                    this.$router.push({path:'/admin/owner/index'})
                    return true
                } else {
