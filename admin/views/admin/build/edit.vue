...
const formState = {
  ...
  build_time: moment(null, 'YYYY-MM-DD'),
  completion_time: moment(null, 'YYYY-MM-DD'),
  lost_reason: null,
...
export default {
  ...
  methods: {
    async getBuild() {
      this.formState = await this.axios.get(`admin/build/${this.id}`)
    },
    next() {
      //提交子组件中的验证
    ...
    onSubmit() {
      ...
      this.axios.put(`admin/build/${this.id}`, this.formState).then(_ => {
